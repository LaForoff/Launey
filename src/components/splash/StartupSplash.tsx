...
export function StartupSplash({ appReady, onRevealStart, onFinish }: StartupSplashProps) {
  ...
  const pathIndexByDot = useMemo(() => {
    const map = new Map<number, number>()

    PATH.forEach((dotIndex, orderIndex) => {
      map.set(dotIndex, orderIndex)
    })
  ...
  return (
    ...
    >
      ...
      <div className="startup-splash__center">
        <div className={isIdle ? 'startup-splash__logo-grid is-idle' : 'startup-splash__logo-grid'}>
          {Array.from({ length: TOTAL_DOTS }, (_, dotIndex) => {
            const pathIndex = pathIndexByDot.get(dotIndex)
            const isVisible = pathIndex !== undefined && pathIndex < builtCount

            return (
              <span
                key={dotIndex}
                className={isVisible ? 'startup-dot is-visible' : 'startup-dot'}
                style={
                  pathIndex !== undefined
                    ? ({
                        '--startup-dot-delay': `${pathIndex * DOT_STAGGER_MS}ms`,
                      } as CSSProperties)
                    : undefined
                }
              />
